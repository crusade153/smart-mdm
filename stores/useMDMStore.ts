...
export const useMDMStore = create<MDMState>((set, get) => ({
  ...
  currentRequest: null,
  selectedIds: [],
  currentUser: { name: '김관리(Admin)', isAdmin: true },
  ...
  toggleUserMode: () => set((state) => ({
    currentUser: {
      name: state.currentUser.isAdmin ? '이요청(User)' : '김관리(Admin)',
